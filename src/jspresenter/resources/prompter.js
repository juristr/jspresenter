$(function() {
  ...
  function adjustPos(elemHeight, cb) {
    ...
    var visibleHeight = $('#js-content').height() + elemHeight;
  ...
  function showOne(cb) {
    var elem = $('#js-content > *:hidden:first');
    if (! firstShown) {
      firstShown = elem;
    }
  ...
  function keepScrolling(cb) {
    ...
    var visibleHeight = parseInt($('#js-content').css('top'), 10);
  ...
  function hideOne(cb) {
    if (stack <= 0) { return; }
    var elem = $('#js-content > *:visible:last');
    var height = elem.outerHeight(true);
